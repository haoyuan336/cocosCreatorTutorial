...
cc.Class({
    ...
    onLoad: function () {
      ...
      global.gameworldEventListener.on("add_combo_effect",this.comboEffect);
      const addEnergy = function (count) {
        cc.log("add enety " + count);
        global.gameData.addEnergyCount(count,()=>{
          ...
          global.gameworldEventListener.fire("add_one_skill_node");

        });
        self.energyProgressBar.progress = global.gameData.getEnergyProgress();
      };
      this.addEnergy = addEnergy;
      global.gameworldEventListener.on("add_energy",this.addEnergy);
    },
    update: function () {
    ...
    onDestroy: function () {
        global.gameworldEventListener.removeListener("add_combo_effect",this.comboEffect);
        global.gameworldEventListener.removeListener("add_energy",this.addEnergy);
      //注销监听
    }
});
